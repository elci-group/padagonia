...
use padagonia::projection::Projection;
...
fn cmd_ingest(args: &[String]) {
    ...
    println!("saved to {} in {:?} (labels={}, relations={})", out, save_time, l, r);
}
...
fn cmd_bfs(args: &[String]) {
    ...
    let store = Store::load(path).expect("load failed");
    let relation_id = relation.as_ref().map(|r| store.string_table.relation_id(r).expect("unknown relation"));
    let engine = QueryEngine::new(&store);
    let start_id = padagonia::NodeId(start);
    let t0 = Instant::now();
...
fn cmd_to_json(args: &[String]) {
    ...
    fs::write(out, serde_json::to_string_pretty(&json).expect("json serialize"))
        .expect("write failed");
    println!("wrote {}", out);
}
...
fn cmd_vector_search(args: &[String]) {
    ...
    let t0 = Instant::now();
    let index = store.build_hnsw_index(distance, 16, 200, ef.max(k));
    let build_time = t0.elapsed();

    let t1 = Instant::now();
    let mut results: Vec<(padagonia::NodeId, f32)> = index
        .search(&query, k, ef.max(k))
        .into_iter()
        .map(|(pid, dist)| (padagonia::NodeId(pid.0), dist))
        .collect();
...
fn cmd_bench_vectors(args: &[String]) {
    ...
    let m: usize = parse_flag(args, "--m").unwrap_or(24);
    let ef_construction: usize = parse_flag(args, "--ef-construction").unwrap_or(200);
    ...
    let query_vecs: Vec<Vec<f32>> =
        (0..queries).map(|_| (0..dim).map(|_| rng.next_f32()).collect()).collect();
    ...
    let t1 = Instant::now();
    let mut hnsw_results: Vec<Vec<padagonia::NodeId>> = Vec::with_capacity(queries);
    for q in &query_vecs {
        let r = index.search(q, k, ef.max(k));
        hnsw_results.push(r.into_iter().map(|(pid, _)| padagonia::NodeId(pid.0)).collect());
    }
    let search_time = t1.elapsed().as_secs_f64();

    // Brute force for recall and baseline timing.
    let t2 = Instant::now();
    let mut brute_results: Vec<Vec<padagonia::NodeId>> = Vec::with_capacity(queries);
    for q in &query_vecs {
